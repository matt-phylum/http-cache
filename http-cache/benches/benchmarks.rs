...
use fake::Fake;
use http_cache::{
    CACacheManager, CacheManager, HttpResponse, HttpVersion, MokaManager,
};
...
async fn test_cacache(input: &str) {
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    for i in inputs {
        group.bench_with_input(
            BenchmarkId::new("cacache", i.0),
            &i.1.as_str(),
            |b, &s| {
                // Insert a call to `to_async` to convert the bencher to async mode.
                // The timing loops are the same as with the normal bencher.
                b.to_async(FuturesExecutor).iter(|| test_cacache(s));
            },
        );
        group.bench_with_input(
            BenchmarkId::new("moka", i.0),
            &i.1.as_str(),
            |b, &s| {
                // Insert a call to `to_async` to convert the bencher to async mode.
                // The timing loops are the same as with the normal bencher.
                b.to_async(FuturesExecutor).iter(|| test_moka(s));
            },
        );
    }
    group.finish();
